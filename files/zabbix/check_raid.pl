...
sub program_names {
	qw(MegaCli64 MegaCli megacli MegaCli_32 megacli_32);
}
...
sub commands {
	{
		...
		'bbuinfo' => ['-|', '@CMD', '-AdpBbuCmd', '-GetBbuCapacityInfo', '-aALL', '-noLog'],
		'cacheinfo' => ['-|', 'MegaCli -LDGetProp  -Cache -LALL -aALL -NoLog | grep "Write Cache OK if bad BBU"']
	}
}
...
sub check {
	...
	my (@status, @devs, @vols, %cur, %cur_vol, $bbustate, $cachestat);
	while (<$fh>) {
		if (my($s) = /Device Id: (\S+)/) {
	...
	$fh = $this->cmd('ldinfo');
	while (<$fh>) {
		if (my($s) = /Name\s*:\s*CacheCade/) {
                        push(@vols, { %cur_vol }) if %cur_vol;
                        %cur_vol = ( name => 'CacheCade', state => 'Optimal', dcache => undef, cache => undef );
                        next;
                }
		if (my($s) = /Name\s*:\s*(\S+)/) {
			push(@vols, { %cur_vol }) if %cur_vol;
			%cur_vol = ( name => $s, state => undef, dcache => undef, cache => undef );
			next;
		}
		if (my($s) = /Name\s*:\s*/) {
                        push(@vols, { %cur_vol }) if %cur_vol;
                        %cur_vol = ( name => 'LD', state => undef, dcache => undef, cache => undef );
                        next;
                }
		if (my($s) = /State\s*:\s*(\S+)/) {
			$cur_vol{state} = $s;
		...
		if (my($s) = /Default Cache Policy\s*:\s*(\S+),\s*/) {
			$cur_vol{dcache} = $s;
			next;
		}
		if (my($s) = /Current Cache Policy\s*:\s*(\S+),\s*/) {
			$cur_vol{cache} = $s;
			next;
		}
	}
	...
	$bbustate = '?' ;
	while (<$fh>) {
		if (my($s) = /Remaining Capacity\s*:\s*(\S+)/) {
			$bbustate = $s ;
			next;
		}
	}
        close $fh;
	$fh = $this->cmd('cacheinfo');
	$cachestat = 'False' ;
	while (<$fh>) {
		if (my($s) = /Write Cache OK if/) {
			$cachestat = 'True' ;
			$this->warning;
			next;
		}
	...
	foreach my $vol (@vols) {
		...
		push(@vstatus, sprintf "%s:%s:%s", $vol->{name}, $vol->{state}, $vol->{cache});
		if ($vol->{state} ne 'Optimal') {
			$this->critical;
		}
		if ($vol->{cache} ne $vol->{dcache}) {
			$this->critical;
		}
	}
	...
	push(@status, ($#vols + 1) . ' Vols: ' . join(',', @vstatus) . ', '. ($#devs + 1) . ' Devs: ' . $this->join_status(\%dstatus) . ', Bbu: ' . $bbustate . ' mAh' . ', CacheBadBBU: ' . $cachestat );
